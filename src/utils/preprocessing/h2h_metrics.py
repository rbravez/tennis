...
def h2h_totals(df: pd.DataFrame) -> pd.DataFrame:
    """Cumulative total H2H matches and win% before each match (directional)."""
    ...
    df['A_H2H_WinPct'] = pd.NA
    df['B_H2H_WinPct'] = pd.NA
    ...
    for i, r in df.iterrows():
        ...
        matches_before = h2h_total_matches[key_a_b]
        wins_before = h2h_total_wins[key_a_b]

        df.loc[i, 'A_H2H_Total_Matches'] = matches_before
        df.loc[i, 'B_H2H_Total_Matches'] = h2h_total_matches[key_b_a]
        df.loc[i, 'A_H2H_WinPct'] = wins_before / matches_before if matches_before > 0 else pd.NA
        df.loc[i, 'B_H2H_WinPct'] = h2h_total_wins[key_b_a] / h2h_total_matches[key_b_a] if h2h_total_matches[key_b_a] > 0 else pd.NA
...
def h2h_surface_totals(df: pd.DataFrame) -> pd.DataFrame:
    """Cumulative total H2H matches and win% on the same surface."""
    df = df.sort_values('tourney_date').reset_index(drop=True)
    ...
    df['A_H2H_Surface_WinPct'] = pd.NA
    df['B_H2H_Surface_WinPct'] = pd.NA
    ...
    for i, r in df.iterrows():
        ...
        matches_before = h2h_surface_matches[key_a_b]
        wins_before = h2h_surface_wins[key_a_b]

        df.loc[i, 'A_H2H_Surface_Matches'] = matches_before
        df.loc[i, 'B_H2H_Surface_Matches'] = h2h_surface_matches[key_b_a]
        df.loc[i, 'A_H2H_Surface_WinPct'] = wins_before / matches_before if matches_before > 0 else pd.NA
        df.loc[i, 'B_H2H_Surface_WinPct'] = h2h_surface_wins[key_b_a] / h2h_surface_matches[key_b_a] if h2h_surface_matches[key_b_a] > 0 else pd.NA
